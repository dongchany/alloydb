...
func newSession(c *C, store kv.Storage, dbName string) alloydb.Session {
...
func sessionExec(c *C, se alloydb.Session, sql string) ([]rset.Recordset, error) {
	se.Execute("BEGIN;")
...
func (s *testTableRsetSuite) TestTableSourceString(c *C) {
	...
	stmtList, err := alloydb.Compile(s.querySql)
	c.Assert(err, IsNil)
