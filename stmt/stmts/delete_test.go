...
func (s *testStmtSuite) TestDelete(c *C) {
	...
	// Test compile
	stmtList, err := alloydb.Compile("DELETE from test where id = 2;")
	c.Assert(err, IsNil)
...
func (s *testStmtSuite) TestMultiTableDelete(c *C) {
	...
	// Test compile
	stmtList, err := alloydb.Compile("DELETE t1, t2 FROM t1 INNER JOIN t2 INNER JOIN t3 WHERE t1.id=t2.id AND t2.id=t3.id;")
	c.Assert(err, IsNil)
	str := stmtList[0].OriginText()
	c.Assert(0, Less, len(str))
